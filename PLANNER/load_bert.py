...
class bert_model(object):
	...
	def predict_label(self, turn, personas_items):
		...
		for input_ids, input_mask, segment_ids, label_ids in eval_dataloader:
			...
			with torch.no_grad():
				#tmp_eval_loss = self.model(input_ids, segment_ids, input_mask, label_ids, return_dict=False)
			...
			# indices_with_zero = [i for i, label in enumerate(labels) if label == 0]
			# elements_at_indices_with_zero = [logit[index] for index in indices_with_zero for logit in logits]

			# Find the maximum value among the elements
			#contradiction_score = max(elements_at_indices_with_zero)

			#element_at_index_with_zero = logits[0][indices_with_zero]
			# return score, logits
